      ...
      <div :key="0" class="table-info">
        <el-table
          ...
          <el-table-column label="到账金额">
            <template slot-scope="scope">
              <div>{{ (scope.row.amount / 100).toFixed(2) + '元' }}</div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="结算状态">
            <template slot-scope="scope">
              ...
              <div class="status status-1" v-if="scope.row.status === 1">已执行</div>
              <div class="status status-2" v-if="scope.row.status === 2">执行失败</div>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    getList () {
      ...
      getRepayOrderList({
        ...
        if (res.data && res.data.code === 0) {
          ...
          this.$message.error(res.data && res.data.message ? res.data.message : '获取结算管理数据失败')
        }
      }).catch(err => {
        console.log(err)
      })
    ...
    gotoDetail (index, info) {
      // 查看详情
      this.$router.push({ name: 'repaymentSettlementDetails', query: { id: info.id } })
    }
  }
}
</script>
